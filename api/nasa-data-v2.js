...
const url = `https://power.larc.nasa.gov/api/temporal/daily/point?parameters=${parameters}&start=${startDate}&end=${endDate}&latitude=${lat}&longitude=${lon}&format=JSON&community=RE`;  const response = await fetch(url, { headers: { Connection: "keep-alive" } });
